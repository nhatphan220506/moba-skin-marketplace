...
export function useProductRoles() {
  ...
  const roles: ProductRole[] = isConnected ? ["PLAYER", "SELLER"] : [];
  ...
  return { address, chainId, isConnected, ready, roles: unique, primaryRole, primaryWorkspace: roleWorkspaces[primaryRole], loading: isConnected && ready && (roleIds.isLoading || checks.isLoading || admin.isLoading) };
}
