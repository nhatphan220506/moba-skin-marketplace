...
import { catalogSourceLabel, featuredDesign, type CatalogItem } from "@/lib/product/catalog";
...
export function ProductCard({ item = featuredDesign, featured = false, compact = false }: { item?: CatalogItem; featured?: boolean; compact?: boolean }) {
  ...
  return <article className={`collectible-card ${featured ? "featured" : ""} ${compact ? "compact-card" : ""}`}>
    ...
    <div className="collectible-copy"><div className="card-status"><span className="live-dot" />{item.status}<small>{catalogSourceLabel(item.source)}</small></div><Link href={`/designs/${item.id}`}><h3>{item.name}</h3></Link><p>{item.description}</p><div className="card-creator"><span className="creator-avatar">{item.artist.slice(0, 2).toUpperCase()}</span><div><small>Created by</small><strong>{item.artist}</strong></div><span className="role-chip">{item.category}</span></div><div className="card-market"><div><small>{item.priceLabel}</small><strong>{item.price}{/^[0-9]+$/.test(item.price) ? " MockVND" : ""}</strong></div><Link href={`/designs/${item.id}`}>View asset <span>↗</span></Link></div></div>
  </article>;
}
