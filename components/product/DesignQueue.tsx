...
export function DesignQueue({ scope = "all", title = "Lifecycle queue" }: { scope?: "owner" | "all"; title?: string }) {
  ...
  const [loading, setLoading] = useState(process.env.NEXT_PUBLIC_STATIC_HOSTING !== "true");
  const [error, setError] = useState("");
  const refresh = useCallback(async () => {
    if (process.env.NEXT_PUBLIC_STATIC_HOSTING === "true") return;
    setLoading(true);
    try {
      const query = scope === "owner" && address ? `?owner=${address}` : "";
      const response = await fetch(`/api/designs${query}`, { cache: "no-store" });
