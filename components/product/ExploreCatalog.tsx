...
export function ExploreCatalog() {
  ...
  useEffect(() => { if (process.env.NEXT_PUBLIC_STATIC_HOSTING === "true") return; fetch("/api/designs", { cache: "no-store" }).then(response => response.ok ? response.json() : Promise.reject()).then((body: { designs: DesignLifecycle[] }) => setLive(body.designs.map(lifecycleToCatalog).filter((item): item is CatalogItem => Boolean(item)))).catch(() => undefined); }, []);
