...
export function ExploreCatalog() {
  ...
  const items = useMemo(() => catalog.filter((item) => `${item.name} ${item.artist} ${item.game} ${item.category}`.toLowerCase().includes(query.toLowerCase()) && (status === "All" || item.status === status)), [query, status]);
